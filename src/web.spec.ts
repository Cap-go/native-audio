...
export class Test {
  ...
  public start(assetId: string, assetPath: string): void {
    this.runLater(
      new TestCase("preload", () =>
        NativeAudio.preload({ assetId: assetId, assetPath: assetPath })
      ),
      new TestCase("duration:", async () =>
        console.log(
          JSON.stringify(await NativeAudio.getDuration({ assetId: assetId }))
        )
      ),
      new TestCase("currentTime:", async () =>
        console.log(
          JSON.stringify(await NativeAudio.getCurrentTime({ assetId: assetId }))
        )
      ),
      new TestCase("play (0)", () =>
